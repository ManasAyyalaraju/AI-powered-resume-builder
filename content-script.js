...
${suffix}`;
  ...
  // src/lib/config.ts
  var SUPABASE_URL = "https://ajchqijmzaxdphlaadeh.supabase.co";
  var SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFqY2hxaWptemF4ZHBobGFhZGVoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY3MTY4MjIsImV4cCI6MjA5MjI5MjgyMn0.0sxDJH1hftzOlsupmmQBmu3DWdWUhSaxvQM9UtdvTAQ";
  var WEB_APP_URL = "https://ai-powered-resume-builder-peach.vercel.app";
  ...
  async function downloadBaseResume(supabase, storagePath) {
  ...
  async function uploadGeneratedResume(supabase, userId, params) {
    try {
      const storagePath = `${userId}/${randomId()}.pdf`;
      const { error: uploadError } = await supabase.storage.from("generated-resumes").upload(storagePath, params.pdfBlob, { contentType: "application/pdf" });
      if (uploadError) throw uploadError;
      const { data, error: insertError } = await supabase.from("generated_resumes").insert({
        user_id: userId,
  ...
  function mountPanelApp({ container, jobContext, onClose }) {
    ...
    function renderPicker() {
      ...
      const options = state.resumes.map(
        (r) => `<option value="${r.id}" ${r.id === state.selectedResumeId ? "selected" : ""}>${escapeHtml(r.file_name ?? r.title)}</option>`
      ).join("");
      return `
      ${jobMeta}
    ...
    function bindEvents() {
      ...
      container.querySelectorAll("[data-format]").forEach((el) => {
        el.addEventListener("click", () => {
          state.resumeFormat = el.dataset.format;
          render();
        });
      });
  ...
  var currentUrl = "";
  var hostEl = null;
  var dismissedForThisPage = false;
  var pollHandle = null;
  function teardown() {
    hostEl?.remove();
    hostEl = null;
  ...
  async function renderPrompt(jobContext) {
